...
st.markdown("""
Aquí encontrarás los onboardings que están finalizados.
""")
...
if df_in_progress.empty:
    ...
    if st.button("Traer últimos onboardings 🔄"):
        ...
        df_in_progress = df[df["Estado"].isin([variables.estados[3], variables.estados[4]])]
        df_in_progress = df[df["Tipo"] == tipo]
    st.stop()
...
with estadoContainer:
    ...
    valor_actual = data_inicial.get("Estado", "Firmado")
    try:
    ...
    with col1:
        ...
        estado = st.selectbox("", variables.estados, index=index_estado, disabled=True)
    with col2:
